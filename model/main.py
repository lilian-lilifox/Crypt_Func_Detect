...
def generate_output_dir(args, base_output_dir="outputs", custom_desc=None):
    feature_name = (
        (basename(args.featuresdir) + "-") if args.featuresdir is not None else ""
    )
...
def iter_configs(args):
    ...
    for desc, params in tunning_params.items():
        ...
        fine_tuning = params.get("is_fine_tuning", False)
        ...
        for idx in range(repeating):
            ...
            log, log_fh = set_logger_filehandler(
                LOG_NAME, args.debug, args.outputdir, "tunning"
            )
            ...
            if testing:
                ...
                config = tunning_params[desc]
            else:
                config = get_config(args)
            update(config, common_config)
            update(config, params)

            yield config, desc, testing, fine_tuning


def model_train(gnn_model, config, desc, fine_tuning=False):
...
def main():
    ...
    parser.add_argument(
        "--inputdir", default="/DBs", help="Path to the Input dir (DBs)"
    )
    ...
    parser.add_argument(
        "-o", "--outputdir", required=False, default=None, help="Output dir"
    )
    ...
    # Iter config items
    for config, desc, testing, fine_tuning in iter_configs(args):
        # Setup GNNModel.
        ...
        if testing:
            ...
            model_train(gnn_model, config, desc, fine_tuning=True)
        else:
