...
def token_mapping(input_folder, output_dir, freq_mode=True):
    ...
    for gtype in GRAPH_TYPES:
        ...
        if os.path.exists(counter_path) and os.path.exists(occurs_path):
            ...
            opc_counters[gtype], opc_occurs[gtype] = (
                dict(
                    [
                        ("opc", Counter()),
                        ("val", Counter()),
    ...
    for gtype, opc_cnts in opc_counters.items():
        ...
        for ty, opc_cnt in opc_cnts.items():
            ...
            if not isinstance(opc_cnt, dict):
                ...
                opc_cnt = sorted(
                    list(opc_cnt.items()), key=lambda k: k[1], reverse=True
                )
            mapped_cnt = 0
            ...
            print(
                "[D] Num of mnemonics mapped: {}".format(len(idmaps[gtype]) - start_id)
            )
...
def create_functions_dict(input_folder, opc_dicts, dump_str, dump_pkl):
    ...
    for idb_path, str_func_one, pkl_func_one in tqdm(
        map(process_one_file, args), total=len(args)
    ):
        if dump_str:
...
def main(input_dir, training, freq_mode, opcodes_json, output_dir, dataset, out_format):
    ...
    if training:
        ...
        opc_dicts = token_mapping(input_dir, output_dir, freq_mode)
        for gtype in GRAPH_TYPES:
            sub_dir = get_sub_dir(output_dir, gtype)
    ...
    str_dict, pkl_dict = create_functions_dict(input_dir, opc_dicts, dump_str, dump_pkl)
